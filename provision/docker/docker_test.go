...
func (s *S) newContainer(opts *newContainerOpts, p *dockerProvisioner) (*container.Container, error) {
	...
	image := "github.com/virtengine/vertice/python:latest"
...
/*func (s *S) TestContainerCreate(c *check.C) {
	...
	s.p.Cluster().PullImage(
		docker.PullImageOptions{Repository: "github.com/virtengine/vertice/brainfuck:latest"},
	...
	c.Assert(container.Config.Env, check.DeepEquals, []string{
		...
		"ABCD=other env",
		"HOST=my.cool:8080",
		"github.com/virtengine/vertice_PROCESSNAME=myprocess1",
	})
}
...
func (s *S) TestGetImageAppWhenDeployIsMultipleOf10(c *check.C) {
	...
	cont := container{ID: "bleble", Type: app.Platform, AppName: app.Name, Image: "github.com/virtengine/vertice/app1"}
	coll := s.p.collection()
...
func (s *S) TestGitDeploy(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(imageId, check.Equals, "github.com/virtengine/vertice/app-myapp:v1")
	var conts []container
	coll := s.p.collection()
	defer coll.Close()
	...
	err = s.p.Cluster().RemoveImage("github.com/virtengine/vertice/app-myapp:v1")
	c.Assert(err, check.IsNil)
}
...
func (s *S) TestGitDeployRollsbackAfterErrorOnAttach(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(conts, check.HasLen, 0)
	err = s.p.Cluster().RemoveImage("github.com/virtengine/vertice/myapp")
	c.Assert(err, check.NotNil)
}
