...
func (s *S) TestCreateContainerForward(c *check.C) {
	err := s.newFakeImage(s.p, "github.com/virtengine/vertice/python", nil)
...
func (s *S) TestCreateContainerBackward(c *check.C) {
	...
	err = s.newFakeImage(s.p, "github.com/virtengine/vertice/python", nil)
	c.Assert(err, check.IsNil)
	defer dcli.RemoveImage("github.com/virtengine/vertice/python")
...
func (s *S) TestAddNewRouteForward(c *check.C) {
	app := provisiontest.NewFakeApp("myapp", "python", 1)
	imageName := "github.com/virtengine/vertice/app-" + app.GetName()
...
func (s *S) TestAddBoxsToHostForward(c *check.C) {
	...
	coll := p.collection()
	defer coll.Close()
	coll.Insert(container{ID: "container-id", AppName: app.GetName(), Version: "container-version", Image: "github.com/virtengine/vertice/python"})
...
func (s *S) TestAddBoxsToHostBackward(c *check.C) {
	...
	coll := s.p.collection()
	defer coll.Close()
	cont := container{ID: "container-id", AppName: app.GetName(), Version: "container-version", Image: "github.com/virtengine/vertice/python"}
	coll.Insert(cont)
	defer coll.RemoveAll(bson.M{"appname": app.GetName()})
	args := changeUnitsPipelineArgs{
		provisioner: s.p,
...
func (s *S) TestFollowLogsAndCommitForward(c *check.C) {
	...
	err = cont.create(runContainerActionsArgs{
		app:         app,
		imageID:     "github.com/virtengine/vertice/python",
		commands:    []string{"foo"},
		provisioner: s.p,
	})
	c.Assert(err, check.IsNil)
	...
	c.Assert(err, check.IsNil)
	c.Assert(imageId, check.Equals, "github.com/virtengine/vertice/app-mightyapp:v1")
	...
	err = s.p.Cluster().RemoveImage("github.com/virtengine/vertice/app-mightyapp:v1")
	c.Assert(err, check.IsNil)
}
