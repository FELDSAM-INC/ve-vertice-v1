...
func (s *S) TestUsePlatformImage(c *check.C) {
	...
	coll := s.p.Collection()
	defer coll.Close()
	err = coll.Insert(container.Container{AppName: app6.Name, Image: "github.com/virtengine/vertice/app-app6"})
	c.Assert(err, check.IsNil)
...
func (s *S) TestPlatformImageName(c *check.C) {
	...
	c.Assert(platName, check.Equals, "github.com/virtengine/vertice/python:latest")
	config.Set("docker:registry", "localhost:3030")
	defer config.Unset("docker:registry")
	platName = platformImageName("ruby")
	c.Assert(platName, check.Equals, "localhost:3030/"github.com/virtengine/vertice/ruby:latest")
}
*/
