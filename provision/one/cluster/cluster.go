...
// Create a node object with a OpenNebula API client
func (c *Cluster) getNodeByObject(nodeo Node) (node, error) {
	var n node
	client, err := api.NewClient(map[string]string{
		api.ENDPOINT: nodeo.Address,
		api.USERID:   nodeo.Metadata[api.USERID],
		api.PASSWORD: nodeo.Metadata[api.PASSWORD],
	})
