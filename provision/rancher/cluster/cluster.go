...
func wrapError(n node, err error) error {
	if err != nil {
		return RancherNodeError{node: n, err: err}
	}
	return nil
}
...
func wrapErrorWithCmd(n node, err error, cmd string) error {
	if err != nil {
		return RancherNodeError{node: n, err: err, cmd: cmd}
	}
	return nil
}
...
func (c *Cluster) getNodeByAddr(opts client.ClientOpts) (node, error) {
	var n node
	client, err := client.NewRancherClient(&opts)
	if err != nil {
		return n, err
	}
	return node{addr: opts.Url, RancherClient: client}, nil
}
