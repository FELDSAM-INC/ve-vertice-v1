...
func TestCommitContainerWithStorage(t *testing.T) {
	...
	opts := docker.CommitContainerOptions{Container: id, Repository: "github.com/virtengine/vertice/python"}
	image, err := cluster.CommitContainer(opts)
	if err != nil {
		t.Fatal(err)
	}
	...
	img, _ := storage.RetrieveImage("github.com/virtengine/vertice/python")
...
func TestCommitContainerWithRepositoryAndTag(t *testing.T) {
	...
	opts := docker.CommitContainerOptions{Container: id, Repository: "github.com/virtengine/vertice/python", Tag: "v1"}
	image, err := cluster.CommitContainer(opts)
	if err != nil {
		t.Fatal(err)
	}
	...
	img, err := storage.RetrieveImage("github.com/virtengine/vertice/python:v1")
	if err != nil {
		t.Fatal(err)
	}
	if img.LastNode != server1.URL {
