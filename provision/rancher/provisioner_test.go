...
func (s *S) TestProvisionerRestart(c *check.C) {
	...
	cont1, err := s.newContainer(&newContainerOpts{
		...
		Image:           "github.com/virtengine/vertice/app-" + app.GetName(),
	}, nil)
	c.Assert(err, check.IsNil)
...
func (s *S) TestDeploy(c *check.C) {
	...
	defer func() { <-stopCh }()
	err := s.newFakeImage(s.p, "github.com/virtengine/vertice/python:latest", nil)
	c.Assert(err, check.IsNil)
	a := app.App{
		Name:     "otherapp",
		Platform: "python",
	...
	err = saveImageCustomData("github.com/virtengine/vertice/app-"+a.Name+":v1", customData)
	c.Assert(err, check.IsNil)
	err = app.Deploy(app."github.com/virtengine{
		App:          &a,
		Version:      "master",
		Commit:       "123",
		OutputStream: w,
...
func (s *S) TestDeployErasesOldImages(c *check.C) {
	...
	err = saveImageCustomData("github.com/virtengine/vertice/app-"+a.Name+":v1", customData)
	c.Assert(err, check.IsNil)
	err = saveImageCustomData("github.com/virtengine/vertice/app-"+a.Name+":v2", customData)
	c.Assert(err, check.IsNil)
	err = app.Deploy(app."github.com/virtengine{
		App:          &a,
		Version:      "master",
	...
	expected := []string{"github.com/virtengine/vertice/app-appdeployimagetest:v1", "github.com/virtengine/vertice/python:latest"}
	...
	got = []string{imgs[0].RepoTags[0], imgs[1].RepoTags[0]}
	sort.Strings(got)
	expected = []string{"github.com/virtengine/vertice/app-appdeployimagetest:v2", "github.com/virtengine/vertice/python:latest"}
	c.Assert(got, check.DeepEquals, expected)
}
...
func (s *S) TestDeployErasesOldImagesIfFailed(c *check.C) {
	...
	s.server.CustomHandler("/containers/create", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err == nil {
			if result.Image == "github.com/virtengine/vertice/app-appdeployimagetest:v1" {
	...
	c.Assert(imgs[0].RepoTags, check.HasLen, 1)
	c.Assert("github.com/virtengine/vertice/python:latest", check.Equals, imgs[0].RepoTags[0])
}
...
func (s *S) TestDeployErasesOldImagesWithLongHistory(c *check.C) {
	...
	err = saveImageCustomData("github.com/virtengine/vertice/app-"+a.Name+":v2", customData)
	c.Assert(err, check.IsNil)
	err = saveImageCustomData("github.com/virtengine/vertice/app-"+a.Name+":v3", customData)
	c.Assert(err, check.IsNil)
	err = app.Deploy(app."github.com/virtengine{
		App:          &a,
		Version:      "master",
	...
	sort.Strings(got)
	expected = []string{"github.com/virtengine/vertice/app-appdeployimagetest:v1", "github.com/virtengine/vertice/app-appdeployimagetest:v2", "github.com/virtengine/vertice/python:latest"}
	c.Assert(got, check.DeepEquals, expected)
	err = app.Deploy(app."github.com/virtengine{
		App:          &a,
		Version:      "master",
	...
	got = []string{imgs[0].RepoTags[0], imgs[1].RepoTags[0], imgs[2].RepoTags[0]}
	sort.Strings(got)
	expected = []string{"github.com/virtengine/vertice/app-appdeployimagetest:v2", "github.com/virtengine/vertice/app-appdeployimagetest:v3", "github.com/virtengine/vertice/python:latest"}
	c.Assert(got, check.DeepEquals, expected)
}
...
func (s *S) TestImageDeploy(c *check.C) {
	err := s.newFakeImage(s.p, "github.com/virtengine/vertice/app-otherapp:v1", nil)
	c.Assert(err, check.IsNil)
	err = appendAppImageName("otherapp", "github.com/virtengine/vertice/app-otherapp:v1")
	c.Assert(err, check.IsNil)
	a := app.App{
		Name:     "otherapp",
		Platform: "python",
	...
	err = app.Deploy(app."github.com/virtengine{
		App:          &a,
		OutputStream: w,
		Image:        "github.com/virtengine/vertice/app-otherapp:v1",
	})
	c.Assert(err, check.IsNil)
	units, err := a.Units()
	c.Assert(err, check.IsNil)
...
func (s *S) TestImageDeployInvalidImage(c *check.C) {
	...
	c.Assert(err, check.ErrorMatches, "invalid image for app otherapp: "github.com/virtengine/vertice/app-otherapp:v1")
	units, err := a.Units()
	c.Assert(err, check.IsNil)
...
func (s *S) TestImageDeployFailureDoesntEraseImage(c *check.C) {
	...
	s.server.CustomHandler("/containers/create", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		err := json.Unmarshal(data, &result)
		if err == nil {
			if result.Image == "github.com/virtengine/vertice/app-otherapp:v1" {
				w.WriteHeader(http.StatusInternalServerError)
				return
			}
		}
	...
	c.Assert(imgs, check.HasLen, 1)
	c.Assert(imgs[0].RepoTags, check.HasLen, 1)
	c.Assert("github.com/virtengine/vertice/app-otherapp:v1", check.Equals, imgs[0].RepoTags[0])
}
...
func (s *S) TestProvisionerDestroyRemovesImage(c *check.C) {
	...
	err = saveImageCustomData(registryURL+"/"github.com/virtengine/vertice/app-"+a.Name+":v1", customData)
	c.Assert(err, check.IsNil)
	err = app.Deploy(app."github.com/virtengine{
		App:          &a,
		Version:      "master",
	...
	c.Assert(registryRequests[0].URL.Path, check.Equals, "/v1/repositories/"github.com/virtengine/vertice/app-mydoomedapp:v1/")
	imgs, err := s.p.Cluster().ListImages(docker.ListImagesOptions{All: true})
	c.Assert(err, check.IsNil)
	c.Assert(imgs, check.HasLen, 1)
	c.Assert(imgs[0].RepoTags, check.HasLen, 1)
	c.Assert(imgs[0].RepoTags[0], check.Equals, registryURL+"/"github.com/virtengine/vertice/python:latest")
}
...
func (s *S) TestProvisionerAddUnits(c *check.C) {
	err := s.newFakeImage(s.p, "github.com/virtengine/vertice/app-myapp", nil)
	c.Assert(err, check.IsNil)
	app := provisiontest.NewFakeApp("myapp", "python", 0)
	app.Deploys = 1
	s.p.Provision(app)
...
func (s *S) TestProvisionerAddUnitsWithErrorDoesntLeaveLostUnits(c *check.C) {
	...
	coll := s.p.Collection()
	defer coll.Close()
	coll.Insert(container.Container{ID: "c-89320", AppName: app.GetName(), Version: "a345fe", Image: "github.com/virtengine/vertice/python:latest"})
...
func (s *S) TestProvisionerRemoveUnits(c *check.C) {
	a1 := app.App{Name: "impius", Teams: []string{"github.com/virtengine/verticeteam", "nodockerforme"}, Pool: "pool1"}
	...
	p := provision.Pool{Name: "pool1", Teams: []string{
		"github.com/virtengine/verticeteam",
		"nodockerforme",
	}}
	...
	err = saveImageCustomData("github.com/virtengine/vertice/app-"+a1.Name, customData)
	c.Assert(err, check.IsNil)
...
func (s *S) TestProvisionerRemoveUnitsEmptyProcess(c *check.C) {
	a1 := app.App{Name: "impius", Teams: []string{"github.com/virtengine/verticeteam"}, Pool: "pool1"}
...
func (s *S) TestProvisionerPlatformAdd(c *check.C) {
	...
	c.Assert(requests[1].URL.Path, check.Equals, "/images/localhost:3030/"github.com/virtengine/vertice/test:latest/json")
	c.Assert(requests[2].URL.Path, check.Equals, "/images/localhost:3030/"github.com/virtengine/vertice/test/push")
}
...
func (s *S) TestRunRestartAfterHooks(c *check.C) {
	...
	err := saveImageCustomData("github.com/virtengine/vertice/python:latest", customData)
	c.Assert(err, check.IsNil)
	err = s.storage.Apps().Insert(a)
	c.Assert(err, check.IsNil)
...
func (s *S) TestShellToAnAppByContainerID(c *check.C) {
	err := s.newFakeImage(s.p, "github.com/virtengine/vertice/app-almah", nil)
	c.Assert(err, check.IsNil)
	app := provisiontest.NewFakeApp("almah", "static", 1)
